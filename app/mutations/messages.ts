...
export const useCreateMessage = defineMutation(() => {
    ...
    const {
        ...
        onSettled: () => {
            useNuxtApp().$createSuccessToast({ title: 'Wiadomość została poprawnie wysłana' })
...
export const useUpdateMessage = defineMutation(() => {
    ...
    const {
        ...
        mutation: (message: Message) =>
            $fetch<SuccessResponse>(`/api/message/${message.id}`, {
                method: 'PATCH',
                body: message,
            }),
        ...
        onMutate: (message) => {
            ...
            if (prevList.length > 0) {
                updatedList = prevList.map((existing) =>
                    existing.id === message.id ? message : message,
                )
                cache.setQueryData(listKey, updatedList)
                cache.cancelQueries({ key: listKey })
            }
            ...
            if (prevDetail !== undefined) {
                cache.setQueryData(detailKey, message)
        ...
        onSettled: (_data, _error, vars, _context) => {
            useNuxtApp().$createSuccessToast({ title: 'Wiadomość została zmieniona poprawnie' })
...
export const useDeleteMessage = defineMutation(() => {
    ...
    const {
        ...
        onSettled: () => {
            useNuxtApp().$createSuccessToast({ title: 'Poprawnie usunięto wiadomość' })
            cache.invalidateQueries({ key: MESSAGE_KEYS.list })
        },
    })
